...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1a = nn.Conv2d(number_of_seq, 32, 8, 4)
    ...
    def forward(self, x):
        ...
        idx = torch.tensor([number_of_seq-1])
...
def train(model, start):
    ...
    state = torch.cat([image_data]*number_of_seq).unsqueeze(0)
    ...
    while iteration < model.number_of_iterations:
        ...
        current_q_values.append(torch.mean(q_value).item())
        current_rewards.append(torch.mean(y_batch).item())
        current_losses.append(loss.item())
        if terminal:
            q_values.append(sum(current_q_values)/float(len(current_q_values)))
            current_q_values = []
            rewards.append(sum(current_rewards)/float(len(current_rewards)))
            current_rewards = []
            losses.append(sum(current_losses)/float(len(current_losses)))
            current_losses = []
