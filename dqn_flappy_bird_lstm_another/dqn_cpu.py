...
class LSTMDQN(nn.Module):
    ...
    def init_states(self) -> [Variable, Variable]:
        hidden_state = Variable(torch.zeros(1, 64, LSTM_MEMORY))
        cell_state = Variable(torch.zeros(1, 64, LSTM_MEMORY))
...
class Agent(object):
    ...
    def train(self, gamma: float = 0.99, mode: str = 'rgb_array'):
        ...
        while True:
            ...
            # logger.info(f'RewardSum:{reward_sum:<3} Q:[{q_mean[0]:<6.4}, {q_mean[1]:<6.4}] ')
            # logger.info(f'T:[{target_mean[0]:<6.4}, {target_mean[1]:<6.4}] ')
            logger.info(f'Epsilon:{self.epsilon:<6.4}{target_update_msg}')
    ...
    def optimize(self, gamma: float):
        ...
        if self.mode == 'dqn':
            ...
            target_pred, self.target_hidden_state, self.target_cell_state = self.target(non_final_next_state_batch,
                                                                                        ...
                                                                                        self.target_cell_state)
        # pdb.set_trace()
        target_values[non_final_mask] = (reward_batch[non_final_mask] + target_pred.max(1)[0] * gamma).unsqueeze(1)
        target_values[final_mask] = reward_batch[final_mask].detach().unsqueeze(1)
    ...
    def toimage(self, image: np.array, name: str):
        Image.fromarray(image, cmin=0, cmax=255).save(name)
